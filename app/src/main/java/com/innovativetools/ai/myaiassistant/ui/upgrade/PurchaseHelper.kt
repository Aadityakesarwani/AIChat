...
data class PurchaseHelper(val activity: Activity) {
    ...
    fun queryProduct(productId: String) {
        ...
        billingClient.queryProductDetailsAsync(params) { billingResult, productDetailsList ->
            if (billingResult.responseCode == BillingClient.BillingResponseCode.OK &&
                ...
                val weeklyPricing =
                    productDetails.subscriptionOfferDetails?.find { it.basePlanId == Constants.WEEKLY_BASE_PLAN }
                        ?.pricingPhases?.pricingPhaseList
                        ?.firstOrNull { it.priceAmountMicros > 0 }
                        ?.formattedPrice

                val yearlyPricing =
                    productDetails.subscriptionOfferDetails?.find { it.basePlanId == Constants.YEARLY_BASE_PLAN }
                        ?.pricingPhases
                        ?.pricingPhaseList
                        ?.firstOrNull { it.priceAmountMicros > 0 }
                        ?.formattedPrice
    ...
    fun makePurchase(proType: ProTypeEnum) {
        coroutineScope.launch {
            // Wait for productDetails to be initialized
            queryProduct(Constants.PRODUCT_ID)
            if (!::productDetails.isInitialized) {
                _statusText.value = "Product details not available yet."
                Log.d("productdetaisl", _statusText.value.toString())
                return@launch
            }

            // Proceed with making the purchase
            val subOfferToken = when (proType) {
                ProTypeEnum.WEEKLY -> productDetails.subscriptionOfferDetails!!.find { it.basePlanId == Constants.WEEKLY_BASE_PLAN }!!.offerToken
                ProTypeEnum.YEARLY -> productDetails.subscriptionOfferDetails!!.find { it.basePlanId == Constants.YEARLY_BASE_PLAN }!!.offerToken
            }
...
//            return
//        }else{
//            Toast.makeText(activity, "not initalized", Toast.LENGTH_SHORT).show()
//        }
//        val subOfferToken = when (proType) {
//            ProTypeEnum.WEEKLY -> productDetails.subscriptionOfferDetails!!.find { it.basePlanId == Constants.WEEKLY_BASE_PLAN }!!.offerToken
////            ProTypeEnum.MONTHLY -> productDetails.subscriptionOfferDetails!!.find { it.basePlanId == Constants.MONTHLY_BASE_PLAN }!!.offerToken
//            ProTypeEnum.YEARLY -> productDetails.subscriptionOfferDetails!!.find { it.basePlanId == Constants.YEARLY_BASE_PLAN }!!.offerToken
//        }
//
//        val billingFlowParams = BillingFlowParams.newBuilder()
